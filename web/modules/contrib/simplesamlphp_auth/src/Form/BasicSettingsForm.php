...
class BasicSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['basic']['anonymous_saml_login_redirect'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Anonymous User SAML Login Redirect'),
      '#default_value' => $config->get('anonymous_saml_login_redirect'),
      '#description' => $this->t('The path you want anonymous users to be redirected after they login via SAML. Leave blank for default (/user).'),
    ];
    $form['basic']['authenticated_saml_login_redirect'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Authenticated User SAML Login Redirect'),
      '#default_value' => $config->get('authenticated_saml_login_redirect'),
      '#description' => $this->t('The path you want already authenticated users to be redirected if they land on SAML login page. Leave blank for default (/admin/content).'),
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config->set('anonymous_saml_login_redirect', $form_state->getValue('anonymous_saml_login_redirect'));
    $config->set('authenticated_saml_login_redirect', $form_state->getValue('authenticated_saml_login_redirect'));
